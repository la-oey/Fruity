...
mypath = 'Exp1/data/FruityStudy_inlab/' #set path to folder containing json files

files = [f for f in listdir(mypath) if isfile(join(mypath, f))]
raw = open('Exp1/inlab/raw.csv','w')
...
for f in files: #iterate through files in folder
	if f != '.DS_Store':
		with io.open('Exp1/data/FruityStudy_inlab/'+f,'r',encoding='utf-8',errors='ignore') as f:
			content = f.read()
